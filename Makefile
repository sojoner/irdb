...
help: ## Show this help message
	...
	@grep -E '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | grep -E '(retag|load|deploy|undeploy|verify-db)' | awk 'BEGIN {FS = ":.*?## "}; {printf "  $(CYAN)%-30s$(NC) %s\n", $$1, $$2}'
	@echo ''
...
.PHONY: deploy-db
deploy-db: verify-operator ## Deploy IR DB instance
	@echo "$(CYAN)Deploying IR DB instance...$(NC)"
	...
	@kubectl apply -k k8s/$(KUSTOMIZE_OVERLAY)
...
undeploy-db: ## Remove IR DB instance
	...
	@if ! kubectl get namespace $(NAMESPACE) >/dev/null 2>&1; then \
		echo "$(YELLOW)Namespace $(NAMESPACE) does not exist$(NC)"; \
		exit 0; \
	fi
	@kubectl delete -k k8s/$(KUSTOMIZE_OVERLAY) --ignore-not-found=true
