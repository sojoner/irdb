...
### Common Configuration Issues (Learnings)

**1. `shared_preload_libraries` Configuration**
- **Issue:** Setting `shared_preload_libraries` in `postgresql.parameters` causes "fixed configuration parameter" error.
- **Fix:** Use the dedicated `postgresql.shared_preload_libraries` list in `values.yaml`.
  ```yaml
  postgresql:
...
**2. Custom Image Version Detection**
- **Issue:** CNPG cannot detect Postgres version from custom tags (e.g., `0.0.8`), causing "can't upgrade between majors" errors.
- **Fix:** Use `imageCatalogRef` with explicit `major` version.
  ```yaml
  imageCatalogRef:
...
**3. Helm Chart Version Logic**
- **Issue:** The upstream chart defaults to Postgres 16. If using 17, the template might incorrectly append `major: 16` to the catalog reference.
- **Fix:** Override the chart version in `values.yaml` (under `cnpg` scope if using dependency).
  ```yaml
  cnpg:
    version:
      postgresql: "17"
...
**4. UID/GID Mismatch**
- **Issue:** Standard Postgres images use UID 999, but CNPG defaults to 26, causing `initdb` permission errors.
- **Fix:** Explicitly set UID/GID in `values.yaml`.
  ```yaml
  cluster:
    postgresUID: 999
    postgresGID: 999
...
**5. `initdb` Configuration Nesting**
- **Issue:** Placing `initdb` under `bootstrap` in `values.yaml` (e.g., `cnpg.cluster.bootstrap.initdb`) causes `postInitApplicationSQL` to be ignored.
- **Fix:** Place `initdb` directly under `cluster`.
  ```yaml
  cluster:
    initdb:
